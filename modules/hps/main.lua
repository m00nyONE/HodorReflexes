-- SPDX-FileCopyrightText: 2025 m00nyONE
-- SPDX-License-Identifier: Artistic-2.0

local addon_name = "HodorReflexes"
local addon = _G[addon_name]
local internal = addon.internal
local core = internal.core

local addon_modules = addon.modules
local internal_modules = internal.modules

local group = core.group
local LGCS = LibGroupCombatStats

local EVENT_PLAYER_HPS_UPDATE = LGCS.EVENT_PLAYER_HPS_UPDATE
local EVENT_GROUP_HPS_UPDATE = LGCS.EVENT_GROUP_HPS_UPDATE

local HR_EVENT_TEST_STARTED = addon.HR_EVENT_TEST_STARTED
local HR_EVENT_TEST_STOPPED = addon.HR_EVENT_TEST_STOPPED
local HR_EVENT_TEST_TICK = addon.HR_EVENT_TEST_TICK

local DAMAGE_UNKNOWN = LGCS.DAMAGE_UNKNOWN

local moduleDefinition = {
    name = "hps",
    friendlyName = GetString(HR_MODULES_HPS_FRIENDLYNAME),
    description = GetString(HR_MODULES_HPS_DESCRIPTION),
    version = "1.0.0",
    priority = 3,
    enabled = false,
    svVersion = 1,
    svDefault = {},

    isTestRunning = false,

    hpsList = nil,
}

local module = internal.moduleClass:New(moduleDefinition)

local playerDataCache = {}
--- handle HPS data received from LGCS
--- @param tag string
--- @param data table
--- @return void
function module:onHPSDataReceived(tag, data)
    if not IsUnitGrouped(tag) then return end

    playerDataCache.name = GetUnitName(tag)
    playerDataCache.tag = tag
    playerDataCache.overheal = data.overheal
    playerDataCache.hps = data.hps

    group.CreateOrUpdatePlayerData(playerDataCache)
end

--- module activation function
--- @return void
function module:Activate()
    self.logger:Debug("activated hps module")

    local registerName = addon_name .. module.name

    -- for testing purposes: /script LibGroupCombatStats.RegisterAddon("test", {"HPS"})
    local lgcs = LGCS.RegisterAddon(registerName, {"HPS"})
    if not lgcs then
        self.logger:Warn("Failed to register %s with LibGroupCombatStats.", registerName)
        return
    end

    lgcs:RegisterForEvent(EVENT_PLAYER_HPS_UPDATE, function(...) self:onHPSDataReceived(...) end)
    lgcs:RegisterForEvent(EVENT_GROUP_HPS_UPDATE, function(...) self:onHPSDataReceived(...) end)

    addon.RegisterCallback(HR_EVENT_TEST_STARTED, function(...) self:startTest(...) end)
    addon.RegisterCallback(HR_EVENT_TEST_STOPPED, function(...) self:stopTest(...) end)
    addon.RegisterCallback(HR_EVENT_TEST_TICK, function(...) self:updateTest(...) end)

    group.RegisterPlayersDataFields({
        overheal = 0,
        hps = 0,
        hideHPS = false,
    })

    self:RunOnce("CreateLists")
end

--- create scrollLists for the module
--- @return void
function module:CreateLists()
    module:RunOnce("CreateHpsList")
end

